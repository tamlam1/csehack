...
class SQL():
    ...
    def createChannel(self, name, category):
        id = self.getMaxChannel() + 1
        self.curr.execute("INSERT INTO channels VALUES (?,?,?)", (id, name, category))
        self.conn.commit()
    
    def getMaxChannel(self):
        id = self.curr.execute("SELECT MAX(CHANNELID) FROM channels")
        for i in id:
            return int(i[0])
